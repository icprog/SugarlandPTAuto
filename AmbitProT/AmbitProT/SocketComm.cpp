...
CSocketComm::CSocketComm() :
    ...
    m_hComm(INVALID_HANDLE_VALUE),m_hRevComm(INVALID_HANDLE_VALUE), m_hThread(NULL), m_hMutex(NULL)
{
}
...
bool CSocketComm::CreateSocketEx(LPCTSTR strHost, LPCTSTR strServiceName, int nFamily, int nType, UINT uOptions /* = 0 */)
{
    ...
    if (INVALID_SOCKET != sock)
    {
		...
		sockAddr.sin_addr.s_addr = htonl(INADDR_ANY);//Add by Jeffrey, support 127.0.0.1 ip address
...
void CSocketComm::StopComm()
{
    ...
    if (IsStart())
    {
        ...
        if (WaitForSingleObject(m_hThread, 1000L/*INFINITE*/) == WAIT_TIMEOUT)
...
DWORD CSocketComm::ReadComm(LPBYTE lpBuffer, DWORD dwSize, DWORD dwTimeout)
{
    ...
    SOCKET s = (SOCKET) m_hRevComm;// Modify by Jeffrey
    // Set Descriptor
...
DWORD CSocketComm::WriteComm(const LPBYTE lpBuffer, DWORD dwCount, DWORD dwTimeout)
{
    ...
    if ( res > 0)
    {
		...
		printf("socket data send:%s\n",lpBuffer);//Add by Maxwell for debug
    }
...
//Run receive thread 
///////////////////////////////////////////////////////////////////////////////
UINT WINAPI SvrRevTask(PVOID pParam)
{

	CSocketComm *pCSocketComm=(CSocketComm *)pParam;
	...
	bool bSmartAddressing = pCSocketComm->IsSmartAddressing();
	if ( !bSmartAddressing )
	{
		lpData = stMsgProxy.byData;
		dwSize = sizeof(stMsgProxy.byData);
	}
	while( pCSocketComm->IsOpen() )
	{
		// Blocking mode: Wait for event
		dwBytes = pCSocketComm->ReadComm(lpData, dwSize, dwTimeout);

		// Error? - need to signal error
		if (dwBytes == (DWORD)-1L)
		{
			// Do not send event if we are closing
			if (pCSocketComm->IsOpen())
			{
				if (bSmartAddressing )
				{
					pCSocketComm->RemoveFromList( stMsgProxy.address );
				}
				pCSocketComm->OnEvent( EVT_CONDROP, &stMsgProxy.address ); //lost connection
			}
		...
		{
			pCSocketComm->OnEvent( EVT_ZEROLENGTH, NULL );
		}
		else if (dwBytes > 0L)
		{
			pCSocketComm->OnDataReceived( lpData, dwBytes);
		}
...
//Run receive thread 
///////////////////////////////////////////////////////////////////////////////
void CSocketComm::CloseRev()
{
    ShutdownConnection((SOCKET)m_hRevComm);
	m_hRevComm=INVALID_HANDLE_VALUE;
...
// IsOneClientConnect
///////////////////////////////////////////////////////////////////////////////
bool CSocketComm::IsOneClientConnect() const
{
    return ( INVALID_HANDLE_VALUE != m_hRevComm );
}

int CSocketComm::BeginRev()
{
	HANDLE hSvrRevThread;
	UINT uiSvrRevThreadId = 0;
	hSvrRevThread = (HANDLE)_beginthreadex(NULL,  // Security attributes
		0,    // stack
		SvrRevTask,   // Thread proc
		this,   // Thread param
		CREATE_SUSPENDED,   // creation mode
		&uiSvrRevThreadId);   // Thread ID

	if ( NULL != hSvrRevThread)
	{
		//SetThreadPriority(hThread, THREAD_PRIORITY_ABOVE_NORMAL);
		ResumeThread( hSvrRevThread );
//		m_hThread = hThread;
		return true;
	}
...
void CSocketComm::Run()
{
	...
	while (1)
	{
		...
		if (IsServer() && !bSmartAddressing)
		{
			...
			{
				SOCKET sock = (SOCKET) m_hComm;
				SOCKET RevSock = WaitForConnection( sock );

				// Get new connection socket
				if (RevSock != INVALID_SOCKET)
				{
					//ShutdownConnection( (SOCKET) m_hComm);//Remove by Jeffrey
					//if (IsOneClientConnect())//make sure only one client connect server
					if (IsOneClientConnect())//Modified by Maxwell
					{
						ShutdownConnection(RevSock);
						RevSock=INVALID_SOCKET;
						continue;
					}

					m_hRevComm = (HANDLE) RevSock;
					OnEvent( EVT_CONSUCCESS, NULL ); // connect sucess event flag
					BeginRev();
				}
				else
				{
