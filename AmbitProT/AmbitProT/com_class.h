...
public:
	...
	bool config_dialog()  
	{
		if(is_open())
		{
			...
			char com_str[10];
			strcpy_s(com_str,sizeof(com_str), "COM");
			_ltoa_s(_port, com_str + 3,sizeof(com_str+3),10);
			
			if(CommConfigDialogA(com_str, NULL, &cf))
			{
	...
	{
		return bool(BuildCommDCBA(set_str, &_dcb));
	}
	//设置内置结构串口参数：波特率，停止位
...
protected:
	...
	virtual void on_receive()
	{
		if(_notify_hwnd)
		{
			...
			//PostThreadMessage(dwPortThreadProcessID, ON_COM_RECEIVE,WPARAM(_port), LPARAM(0));
		}
...
private:
	...
	static DWORD WINAPI com_thread(LPVOID para)
	{
		...
		for(DWORD length, mask = 0; pcom->_run_flag && pcom->is_open(); mask = 0)
		{
			...
			{
				ClearCommError(pcom->_com_handle, &error, &stat);
				if((unsigned)stat.cbInQue >(unsigned) pcom->_notify_num)
					pcom->on_receive();
			}
        }
